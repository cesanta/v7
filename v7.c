 ...
 */
 typedef struct
 {
   uint8_t _dummy[((cr_locals_size_t)-1)];
 } cr_zero_size_type_t;
...
#define CR_CALL_C(p_ctx, call_fid, local_ret_fid)                            \
  ...
  {                                                                          \
    ...
    goto _cr_iter_begin;                                                     \
  local_ret_fid: /* we'll get here when called function returns */           \
                 ;                                                           \
  } while (0)
...
#define CR_YIELD_C(p_ctx, value, local_ret_fid)                        \
  ...
  {                                                                    \
    ...
    CR_CURR_FUNC_C(p_ctx) = (local_ret_fid);                           \
                                                                       \
    goto _cr_iter_begin;                                               \
  local_ret_fid: /* we'll get here when the machine will be resumed */ \
                 ;                                                     \
  } while (0)
...
#define V7_CHECK(v7, COND)                                      \
  ...
  {                                                             \
    if (!(COND))                                                \
      throw_exception(v7, INTERNAL_ERROR, "line %d", __LINE__); \
  } while (0)
#endif
 ...
 */
   ...
   case V7_TYPE_REGEXP_OBJECT:
   {
     ...
     if (flags & SLRE_FLAG_G)
       s2[n2++] = 'g';
     if (flags & SLRE_FLAG_I)
       s2[n2++] = 'i';
     if (flags & SLRE_FLAG_M)
       s2[n2++] = 'm';
   ...
   case V7_TYPE_ERROR_OBJECT:
   {
     ...
     while ((h = v7_next_prop(h, v, &name, &val, &attrs)) != NULL)
     {
       ...
       if (b - buf != 1)
       { /* Not the first property to be printed */
         b += c_snprintf(b, BUF_LEFT(size, b - buf), ",");
       }
       s = v7_to_string(v7, &name, &n);
   ...
   if (v7_is_object(arr))
   {
     if (v7_to_object(arr)->attributes & V7_OBJ_DENSE_ARRAY)
     {
       ...
       else
       {
         ...
         if (has != NULL && res != V7_TAG_NOVALUE)
           *has = 1;
 ...
 */
 ...
 struct v7 *v7_create_opt(struct v7_create_opts opts)
 {
   ...
   if (opts.object_arena_size == 0)
     opts.object_arena_size = 200;
   if (opts.function_arena_size == 0)
     opts.function_arena_size = 100;
   if (opts.property_arena_size == 0)
     opts.property_arena_size = 4000;
...
/* unlikely but this could be a string as well */
 ...
 void __cyg_profile_func_enter(void *this_fn, void *call_site)
 {
   ...
   if (profile_enter || v7_sp_limit == NULL)
     return;

   profile_enter++;
   if (v7_head != NULL && fp < v7_head->sp_lwm)
     v7_head->sp_lwm = fp;
...
#ifdef V7_BROKEN_NAN
   if (isnan(a) || isnan(b))
     return tag == AST_NE || tag == AST_NE_NE;
#endif

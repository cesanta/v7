...
static val_t Function_ctor(struct v7 *v7, val_t this_obj, val_t args) {
  ...
  exec_str = (char *) malloc(size + 1);
  if (exec_str == NULL)
    throw_exception(v7, INTERNAL_ERROR,
                    "not enough memory for construct function");
  memcpy(exec_str, s, size);
  exec_str[size] = '\0';

  if (v7_exec(v7, &res, exec_str) != V7_OK) {
    free(exec_str);
    throw_exception(v7, SYNTAX_ERROR, "Invalid function tmp");
  }
  free(exec_str);

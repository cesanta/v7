 ...
 *   `num_subtrees` is zero, and interpreter handles child nodes one by one,
 *   until the end of the node is reached (end of the node is determined by the
 *   `end` skip)
 *
 *
 * Examples:
 ...
 *
 *    - 00
 *        This extra trailing byte at the end of the script is needed by
 ...
 * Tree:
 *
 *    $ ./v7 -e 'var foo=0, bar=1; foo=3; var baz = 4;' -t
 *    SCRIPT
 *      /- [...] -/
 *      VAR
 *        /- [...] -/
 ...
 * Binary:
 *
 *    $ ./v7 -e 'var foo=0, bar=1; foo=3; var baz = 4;' -b | od -A n -t x1
 *    56 07 41 53 54 56 31 30 00 01 00 2f 00 05 02 00
 *    14 00 1e 03 03 66 6f 6f 13 01 30 03 03 62 61 72
 *    13 01 31 07 14 03 66 6f 6f 13 01 33 02 00 0c 00
 *    00 03 03 62 61 7a 13 01 34 00
 *
 * Break it down:
 *
 *    - 56 07 41 53 54 56 31 30 00
 ...
 *    - 00 2f:    `end` skip: 0x2f = 47 bytes
 *    - 00 05:    `next` skip: an offset from `AST_SCRIPT` byte to the first
 *                `var` declaration.
 *
 *        Now, body of the SCRIPT node begins, which contains child nodes,
 *        and the first node is the var declaration `var foo=0, bar=1;`:
 *
 *        SCRIPT node body: {{{
 *    - 02:       AST tag: `AST_VAR`
 *    - 00 14:    `end` skip: 20 bytes from tag byte to the end of current node
 *    - 00 1e:    `next` skip: 30 bytes from tag byte to the next `var` node
 *
 *        The VAR node contains arbitrary number of child nodes, and interpreter
 *        takes advantage of the `end` skip.
 *
 *        VAR node body: {{{
 *    - 03:       AST tag: `AST_VAR_DECL`
 *    - 03:       Varint value: 3 (the length of the inlined data: a variable name)
 *    - 66 6f 6f: ASCII data: "foo"
 *    - 13:       AST tag: `AST_NUM`
 *    - 01:       Varint value: 1
 *    - 30:       ASCII string "0"
 *
